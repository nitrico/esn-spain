...
public class TagHandler implements Html.TagHandler {
  ...
  public void handleTag(boolean opening, String tag, Editable output, XMLReader xmlReader) {
    if (tag.equalsIgnoreCase("ul")) {
      ...
      if (opening) {
        lists.push(tag);
        olNextIndex.push(Integer.valueOf(1)).toString(); // TO DO: add support for lists starting other index than 1
      }
      else {
        lists.pop();
      ...
      if (opening) {
        ...
        else if (parentList.equalsIgnoreCase("ul")) start(output, new Ul());
      }
      else {
        if (lists.peek().equalsIgnoreCase("ul")) {
          ...
          if (lists.size() > 1) {
            ...
            if (lists.size() > 2) bulletMargin -= (lists.size()-2) * listItemIndent;
          }
          ...
          if (lists.size() > 2) numberMargin -= (lists.size()-2) * listItemIndent;
          end(output, Ol.class, new LeadingMarginSpan.Standard(numberMargin));
        }
      }
    }
    else if (opening && !tag.toUpperCase().equals("HTML") && !tag.toUpperCase().equals("BODY"))
